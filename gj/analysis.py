...
API_KEY = "7WsIldOBwicwq35gSg1Qdg9GVcgq9Czv88wQfyI9ilA"
...
def prepare_data (input_filename, delimiter=";", encoding='utf-8'):
    ...
    while list_to_geocode:
        ...
        fg = ForwardGeocoderV7(list_to_geocode[:5], API_KEY)

        # Run geocoding
        results += asyncio.run(fg.main())

        del list_to_geocode[:5]
...
def to_csv():
    ...
    with open('data.csv','w') as f:
        ...
        writer.writerow(['ADDRESS', 'NAME', 'ORIGINAL_LAT', 'ORIGINAL_LNG'])

        for item in data["features"]:
            writer.writerow([item["properties"]["address"] if item["properties"]["address"] else item["properties"]["name"] , item["properties"]["name"], item["geometry"]["coordinates"][1], item["geometry"]["coordinates"][0]])
